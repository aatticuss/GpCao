...
async function loadExercises() {
    ...
    if (!token) {
        container.innerHTML = '<p class="text-red-600 col-span-full">Usuário não autenticado.</p>';
        return;
    }
    ...
    try {
        ...
        if (!response.ok) {
            ...
            container.innerHTML = `<p class="text-red-600 col-span-full">Erro: ${errorData.error || 'Não foi possível carregar os exercícios.'}</p>`;
            return;
        }

        const data = await response.json();
        const exercises = data.exercicios;
        ...
        exercises.forEach(ex => {
            ...
            switch(ex.dificuldade) {
                case 'facil': bgColor = 'green-100'; textColor = 'green-800'; break;
                case 'medio': bgColor = 'yellow-100'; textColor = 'yellow-800'; break;
                case 'dificil': bgColor = 'red-100'; textColor = 'red-800'; break;
                default: bgColor = 'gray-100'; textColor = 'gray-800';
            }
            ...
            container.innerHTML += `
                <a href="/exercises/${ex.id}" class="block bg-white border border-gray-200 rounded-lg shadow-sm p-5 hover:shadow-md">
                    ...
                    <div class="flex flex-wrap items-center justify-between text-gray-500 text-xs">
                        ${ex.technology ? `<span class="bg-gray-100 text-gray-800 px-2 py-1 rounded-full">${ex.technology.nome}</span>` : ''}
                        <span class="bg-${bgColor} text-${textColor} px-2 py-1 rounded-full">${ex.dificuldade.charAt(0).toUpperCase() + ex.dificuldade.slice(1)}</span>
        ...
        container.innerHTML = `<p class="text-red-600">Erro: ${error.message}</p>`;
    }
}
